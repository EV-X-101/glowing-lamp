...
motor_pwm = GPIO.PWM(enable_pin, 10000)  # Set PWM frequency to 1000 Hz
motor_pwm.start(0)
...
def encoder_callback(channel):
    ...
    if a_value == b_value:
        encoder_position += 1
        print(encoder_position,"if equL")
    else:
        encoder_position -= 1
        print(encoder_postion, "not equL")
...
try:
    while True:
        ...
        ticks_per_degree = 1  # Adjust based on encoder resolution
        ...
        while abs(encoder_position) < abs(ticks):
            time.sleep(0.1)
            print(encoder_position, ticks)
        # Stop the motor
        set_motor_speed(0)
        
        # Return to zero position
        encoder_position = 0
        set_motor_speed(100 if encoder_position < 0 else -100)
