...
def run_motor():
    ...
    while True:
        ...
        with target_degree_lock:
            if target_degree == 0:
                encoder_center = position  # Set the center to current position
...
try:
    while True:
        ...
        with target_degree_lock:
            ...
            if target_degree == 0:  # If target degree is set to 0, reset the encoder position
                with encoder_lock:
                    encoder_pos = encoder_center
finally:
    pwm.stop()
    GPIO.cleanup()
