...
def run_motor():
    ...
    pid = PID(1, 0.3, 0.05, setpoint=0)
    pid.output_limits = (-1, 1)
    while True:
        with encoder_lock:
            position = encoder_pos
        with target_degree_lock:
            pid.setpoint = target_degree  # Set the target degree relative to the center
        output = pid(position - encoder_center)  # Adjust the input to the PID controller to be relative to the center
